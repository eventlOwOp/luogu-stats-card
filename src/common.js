...
class Card {
	...
	render() {
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="${
				cardSize.width
			}" height="${cardSize.height}" viewBox="0 0 ${cardSize.width} ${
			cardSize.height
		}" fill="none">
        <style>
          .text { font: 400 11px 'Segoe UI', Ubuntu, Sans-Serif; fill: ${
						this.darkMode ? "#fffefe" : "#333333"
					} }
	  ...
	  text.gray { fill: #aaa; }
        ...
        <g transform="translate(${this.paddingX}, ${
...
const renderCCFBadge = (level, x) => {
	...
	return `
  <svg xmlns="http://www.w3.org/2000/svg" x="${x}" y="-14" width="18" height="18" viewBox="0 0 18 18" fill="${ccfColor(
		level
 ...
 */
const renderChart = (datas, labelWidth, progressWidth, unit) => {
	//(label, color, height, num, unit) => {
	let chart = "";
    ...
    <g transform="translate(0, ${i * 30})">
      ...
      <text x="${width + labelWidth + 10}" y="15" class="text">${
			datas[i].data + unit
		}</text>
      <rect height="11" fill="${
				datas[i].color
			}" rx="5" ry="5" x="${labelWidth}" y="5" width="${width}"></rect>
    </g>
    `;
	}
	...
	for (let i = 0; i <= 4; ++i) {
		coordinate += `
    <line x1="${labelWidth + dw * i}" y1="0" x2="${labelWidth + dw * i}" y2="${
			bodyHeight - 10
		}"  class="line"/>
    <text x="${
			labelWidth + dw * i - (i == 0 ? 3 : 5)
		}" y="${bodyHeight}"  class="text">${(maxNum * i) / 4}</text>
    `;
	}
...
const renderNameTitle = (name, color, ccfLevel, title, cardWidth, rightTop) => {
  ...
  <g transform="translate(0, 0)" font-family="Verdana, Microsoft Yahei" text-rendering="geometricPrecision" font-size="18">
    ...
    <text x="${
			nameLength + (ccfLevel < 3 ? 10 : 28)
		}" y="0" class="title" font-weight="normal">
      ${title}
    </text>
    <text x="${
			cardWidth - 210
		}" y="0" class="title" font-weight="normal" font-size="13px">
      ${rightTop}
    </text>
  </g>`;
};
